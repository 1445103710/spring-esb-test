...
public class OSSBootUtil {
     ...
     * @return oss 中的相对文件路径
     */
    public static String upload(AliyunOSSConfig ossConfig, InputStream in, String fileDir, String fileName){
        initOSS(ossConfig);
...
//            String fileName = System.currentTimeMillis() + "-" + UUID.randomUUID().toString().substring(0,18) + suffix;
            ...
            ossClient.putObject(ossConfig.getBucketName(), fileUrl.toString(), in);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
