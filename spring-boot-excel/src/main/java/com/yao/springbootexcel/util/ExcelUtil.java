package com.yao.springbootexcel.util;
...
//    }
    ...
    public static void copyRows(Sheet sourceSheet, Sheet targetSheet, int pStartRow, int pEndRow) {
        Row rowx = sourceSheet.getRow(pStartRow);
        Row rowy = targetSheet.createRow(pEndRow);
        short lastCellNumx = rowx.getLastCellNum();
        for (int i = 0; i < lastCellNumx; i++) {
            Cell cellx = rowx.getCell(i);
            Cell celly = rowy.createCell(i);
            if (!Objects.isNull(cellx)) {
                celly.getCellStyle().cloneStyleFrom(cellx.getCellStyle());
                try {
...
//                            System.out.println("空空如也");
                            ...
                            break;
                        case STRING:
                            celly.setCellValue(cellx.getStringCellValue());
                            celly.setCellType(cellx.getCellType());
                            break;
                        case NUMERIC:
                            celly.setCellValue(cellx.getNumericCellValue());
                            celly.setCellType(cellx.getCellType());
                            break;
                        case FORMULA:
